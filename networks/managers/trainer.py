...
from dataloaders.train_datasets import DAVIS2017_Train, YOUTUBEVOS_Train, VIPSEG_Train, StaticTrain,TEST,BURST_Train,MOTS_Train,OVIS_Train
...
from utils.learning import adjust_learning_rate, get_trainable_params, select_trainable_params
...
class Trainer(object):
    def __init__(self, rank, cfg, enable_amp=True):
        ...
        if hasattr(cfg, 'ENABLE_REFINE') and cfg.ENABLE_REFINE:
            trainable_params = select_trainable_params(trainable_params)
    ...
    def prepare_dataset(self):
        ...
        train_datasets = []
        if 'sam' in cfg.DATASETS:
            print('####################################use sam for training')
            pretrain_sam_dataset = SAMTrain(
                cfg.DIR_SAM,
                cfg.DATA_RANDOMCROP,
            ...
            train_datasets.append(pretrain_sam_dataset)
            self.enable_prev_frame = False
        if 'static' in cfg.DATASETS:
            print('####################################use static for training')
            ...
            self.enable_prev_frame = False
        if 'coco' in cfg.DATASETS:
            print('####################################use coco for training')
            pretrain_vos_dataset = COCOTrain(
                cfg.DIR_STATIC,
                cfg.DATA_RANDOMCROP,
                seq_len=cfg.DATA_SEQ_LEN,
                merge_prob=cfg.DATA_DYNAMIC_MERGE_PROB,
        ...
        if 'davis2017' in cfg.DATASETS:
            print('####################################use davis2017 for training')
        ...
        if 'youtubevos' in cfg.DATASETS:
            print('####################################use youtubevos for training')
            train_ytb_dataset = YOUTUBEVOS_Train(
                root=cfg.DIR_YTB,
                transform=composed_transforms,
                repeat_time=cfg.DATA_YTB_REPEAT,
                seq_len=cfg.DATA_SEQ_LEN,
        ...
        if 'bl30k' in cfg.DATASETS:
            print('####################################use bl30k for training')
            train_bl30k_dataset_split_a = BL30K_Train(
                transform=composed_transforms,
                seq_len=cfg.DATA_SEQ_LEN,
                ...
                max_obj_n=cfg.MODEL_MAX_OBJ_NUM,
                split='a')
            train_datasets.append(train_bl30k_dataset_split_a)
            train_bl30k_dataset_split_b = BL30K_Train(
                transform=composed_transforms,
                seq_len=cfg.DATA_SEQ_LEN,
                rand_gap=cfg.DATA_RANDOM_GAP_VIP,
                rand_reverse=cfg.DATA_RANDOM_REVERSE_SEQ,
                ...
                max_obj_n=cfg.MODEL_MAX_OBJ_NUM,
                split='b')
            train_datasets.append(train_bl30k_dataset_split_b)
            train_bl30k_dataset_split_c = BL30K_Train(
                transform=composed_transforms,
                seq_len=cfg.DATA_SEQ_LEN,
                rand_gap=cfg.DATA_RANDOM_GAP_VIP,
                rand_reverse=cfg.DATA_RANDOM_REVERSE_SEQ,
                ...
                max_obj_n=cfg.MODEL_MAX_OBJ_NUM,
                split='c')
            train_datasets.append(train_bl30k_dataset_split_c)

        if 'ovis' in cfg.DATASETS:
            print('####################################use ovis for training')
        ...
        if 'mots' in cfg.DATASETS:
            print('####################################use mots for training')
            train_mots_dataset = MOTS_Train(
                transform=composed_transforms,
                seq_len=cfg.DATA_SEQ_LEN,
                rand_gap=cfg.DATA_RANDOM_GAP_VIP,
                rand_reverse=cfg.DATA_RANDOM_REVERSE_SEQ,
                ...
                enable_prev_frame=self.enable_prev_frame,
                max_obj_n=cfg.MODEL_MAX_OBJ_NUM)
            train_datasets.append(train_mots_dataset)

        if 'burst' in cfg.DATASETS:
            print('####################################use burst for training')
            train_burst_dataset = BURST_Train(
                transform=composed_transforms,
                seq_len=cfg.DATA_SEQ_LEN,
                rand_gap=cfg.DATA_RANDOM_GAP_VIP,
                rand_reverse=cfg.DATA_RANDOM_REVERSE_SEQ,
                ...
                enable_prev_frame=self.enable_prev_frame,
                max_obj_n=cfg.MODEL_MAX_OBJ_NUM)
            train_datasets.append(train_burst_dataset)

        if 'vipseg' in cfg.DATASETS:
            print('####################################use vipseg for training')
