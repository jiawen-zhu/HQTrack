...
class BL30K_Train(VOSTrain):
    def __init__(self,
        ...
        root=os.path.join(root,'BL30K_'+split)
        ...
        for seq_name in seq_list:
            ...
            for img_file in os.listdir(os.path.join(image_root,seq_name)):
                images.append(img_file)
                labels.append(img_file.replace('.jpg','.png'))
...
class OVIS_Train(VOSTrain):
    def __init__(self,
        ...
        super(OVIS_Train, self).__init__(image_root,
                                               label_root,
                                               imglistdic,
                                               transform,
                                               rgb,
...
class MOTS_Train(VOSTrain):
    def __init__(self,
        ...
        super(MOTS_Train, self).__init__(image_root,
                                               label_root,
                                               imglistdic,
                                               transform,
                                               rgb,
...
class BURST_Train(VOSTrain):
    def __init__(self,
        ...
        super(BURST_Train, self).__init__(image_root,
                                               label_root,
                                               imglistdic,
                                               transform,
                                               rgb,
...
class VIPSEG_Train(VOSTrain):
    def __init__(self,
        ...
        label_root = os.path.join(root, 'panomasks_wostuff')

        # read anno info
        seq_list_file = os.path.join(root, 'panoptic_gt_VIPSeg_train.json')
        seqs_info = json.load(open(seq_list_file, 'r'))['annotations']

        # for video in videos
        imglistdic = {}
        for i in range(len(seqs_info)):
            data = seqs_info[i]
            seq_name = data['video_id']

            images = []
            labels = []
            for frame in data['annotations']:
                seg_info = frame['segments_info']
                if len(seg_info) > 0:
                    images.append(frame['file_name'].replace('.png','.jpg'))
                    labels.append(frame['file_name'])
            images = np.sort(np.unique(images))
            labels = np.sort(np.unique(labels))
            if len(images) < 2:
                print("Short video: " + seq_name)
        ...
        super(VIPSEG_Train, self).__init__(image_root,
                                               label_root,
                                               imglistdic,
                                               transform,
                                               rgb,
